import { File, AlignmentType, LevelFormat, UnderlineType, Packer, Paragraph, BorderStyle, ShadingType, TextRun, HeadingLevel, TableCell, WidthType, TableRow, Table, ImageRun, ExternalHyperlink, Math$1, MathRun, MathFraction, MathRadical, MathSuperScript, MathSubScript, MathSubSuperScript, MathRoundBrackets, MathSquareBrackets, MathCurlyBrackets, MathAngledBrackets } from './index4.js';
import { marked } from './marked.esm.js';
import markedKatex from './index3.js';
import { renderMathInElement } from './Markdown.js';
import './katex.js';
...
function mathComponentsFromMathNode(node) {
  ...
  if (tag === "math" || tag === "mrow" || tag === "mstyle" || tag === "semantics") {
    ...
    if (!kids.length) {
      const t2 = normalizeMathText(node.textContent || "");
      return t2 ? [new MathRun(t2)] : [];
    }
    return kids;
  }
  ...
  for (const li of items) {
    ...
    out.push(
      paragraphFromInlineNodes(inlineNodes, {
        ...
        numbering: ordered ? { reference: ORDERED_LIST_REF, level } : void 0,
        indent: { left, hanging },
        spacing: { after: 180 }
      })
    );
      ...
      const cellRuns = await Promise.all(
        cells.map(async (cellEl) => {
          ...
          return new TableCell({
            ...
            shading: isHeader ? { type: ShadingType.CLEAR, fill: COLORS.thBg, color: "auto" } : void 0,
            children
          });
        })
      );
...
function decodeBase64ToBytes(base64) {
  ...
  if (typeof Uint8Array.fromBase64 === "function") {
    return Uint8Array.fromBase64(s, { alphabet: "base64", lastChunkHandling: "loose" });
  }
...
async function imageRunFromSrc(src, alt) {
  ...
  if (data) {
    ...
    return new ImageRun({
      ...
      altText: alt ? { title: alt, description: alt, name: alt } : void 0
    });
  }
  try {
...
async function convertBlockNode(node, listLevel, ctx) {
  ...
  if (tag === "p") {
    ...
    const hasOnlyImg = imgs.length === 1 && el.childNodes.length === 1 && el.firstElementChild?.tagName.toLowerCase() === "img";
    if (hasOnlyImg) {
  ...
  if (heading) {
    ...
    const wantsPageBreakBefore = ctx.chapterByH1 && isH1 && ctx.blockIndex > 0 || el.classList.contains("print-chapter") || el.classList.contains("print-break-before");
    const withBorder = heading === HeadingLevel.HEADING_1 || heading === HeadingLevel.HEADING_2 ? {
  ...
  if (tag === "blockquote") {
    ...
    for (const child of Array.from(el.childNodes)) {
      if (child.nodeType === Node.TEXT_NODE) {
        const t2 = (child.nodeValue ?? "").trim();
        if (t2) out.push(new Paragraph({ ...quoteDecor, children: [textRun(t2, {})] }));
        continue;
      }
      ...
      const heading2 = headingLevelFromTag(ct);
      if (heading2) {
        out.push(
...
async function createDocxBlobFromHtml(html, options = {}) {
  ...
  const children = [];
  const ctx = { blockIndex: 0, chapterByH1: true };
...
async function downloadHtmlAsDocx(html, options = {}) {
  const title = options.title || "Document";
  const filename = options.filename || `${safeFilename(title)}.docx`;
...
export { createDocxBlobFromHtml, createDocxBlobFromMarkdown, downloadHtmlAsDocx, downloadMarkdownAsDocx };
