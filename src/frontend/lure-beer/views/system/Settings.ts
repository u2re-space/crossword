...
export const Settings = async () => {
    ...
    const saveBtn = H`<button type="submit" class="btn save"><ui-phosphor-icon icon="check"></ui-phosphor-icon><span>Save changes</span></button>` as HTMLButtonElement;
        ...
        const container = H`<div class="mcp-config ${isNew ? 'mcp-config-new' : ''}" data-mcp-id=${mcpConfig.id}>
            <div class="mcp-header">
                ...
                <button type="button" class="btn btn-danger btn-sm remove-mcp" data-mcp-id=${mcpConfig.id}>
                    <ui-phosphor-icon icon="trash"></ui-phosphor-icon>
                    <span>Remove</span>
                </button>
            </div>
    ...
    SETTINGS_SECTIONS.forEach((section) => {
        const button = H`<button type="button" class="settings-tab" role="tab" id=${`tab-${section.key}`} aria-controls=${`panel-${section.key}`} aria-selected="false">
            <ui-phosphor-icon icon=${section.icon}></ui-phosphor-icon>
        ...
        section.groups.forEach((group) => {
            ...
            if (section.key === 'mcp' && group.key === 'mcp-management') {
                ...
                const addButton = H`<button type="button" class="btn btn-primary add-mcp">
                    <ui-phosphor-icon icon="plus"></ui-phosphor-icon>
    ...
    const tabsNav = H`<div class="tabs-nav">
        ${Array.from(navButtons.values())}
    </div>` as HTMLElement;
    
    const tabsContent = H`<div class="tabs-content">
        ${Array.from(panelRefs.values())}
    </div>` as HTMLElement;
    
    const panelsWrapper = H`<div class="tabs-container" style="background-color: transparent; inline-size: stretch; block-size: stretch;">
        ${tabsNav}
        ${tabsContent}
    </div>` as HTMLElement;
