...
import { CHANNEL_NAMES, postMessage, affected } from "../../core/utils/Broadcast";
import { MAX_FILE_SIZE } from "./model/GPT-Responses";
// stringToBlob replaced with Node.js equivalent
const stringToBlob = (dataUrl) => {
    const match = dataUrl.match(/^data:([^;]+);([^,]+),(.+)$/);
    if (!match) return null;
    const [, mimeType, encoding, data] = match;
    const buffer = Buffer.from(data, encoding === "base64" ? "base64" : "utf8");
    return { type: mimeType, buffer, size: buffer.length, arrayBuffer: () => buffer.buffer.slice(buffer.byteOffset, buffer.byteOffset + buffer.byteLength) };
};
