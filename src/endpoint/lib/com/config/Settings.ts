...
import { readFile as fsReadFile, stat, readdir, mkdir } from "node:fs/promises"
...
// Backend-compatible replacements for fest/lure functions
const getDirectoryHandle = async (rootHandle: string | null, path: string, options?: { create?: boolean }) => {
    try {
        const fullPath = rootHandle ? join(rootHandle, path) : path;
        if (options?.create) {
...
const readFile = async (rootHandle: string, fullPath: string) => {
    try {
        const path = rootHandle ? join(rootHandle, fullPath) : fullPath;
